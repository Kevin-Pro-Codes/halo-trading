...
function App() {
  ...
  useEffect(() => {
    ...
    const fetchBtcPrice = () => {
      ...
      const randomFluctuation = Math.random() * 2000 - 1000; // +/- $1000
      const newPrice = 50000 + randomFluctuation;
      setBtcPrice(newPrice);
      setUsdAmount(btcAmount * newPrice);
    };
  ...
  const handleBtcChange = (e) => {
    ...
    setBtcAmount(isNaN(value) ? 0 : value);
    setUsdAmount((isNaN(value) ? 0 : value) * btcPrice);
  };

  const handleUsdChange = (e) => {
    const value = parseFloat(e.target.value);
    setUsdAmount(isNaN(value) ? 0 : value);
    setBtcAmount((isNaN(value) ? 0 : value) / btcPrice);
  };
